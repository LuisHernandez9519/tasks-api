...
import { StatusEnum, StatusSubTaskEnum, StatusTaskEnum } from "src/common/enums";
...
export class SubTaskService {
    ...
    async createSubtask(data: CreateSubTaskDto): Promise<SubTask>{
        ...
        if (!task) {
            throw new NotFoundException(`resource task with id=${data.task_id} not found`)
        }
    ...
    async updateSubtask(data: UpdateSubTaskDto, id: number): Promise<SubTask>{
        ...
        const subtaksUpdate = this.subtaskRepository.create({...subtask, ...data})
